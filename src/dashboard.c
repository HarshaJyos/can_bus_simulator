...
void dashboard_init(void) {
    memset(&live_data, 0, sizeof(live_data));
}

void dashboard_update(can_frame_t frame) {
    if (frame.id == ECU_ID_ENGINE) {
        live_data.rpm = dbc_decode(frame, SIG_ENGINE_RPM);
    } else if (frame.id == ECU_ID_ENGINE + 1) {
        live_data.temp = dbc_decode(frame, SIG_ENGINE_TEMP);
        live_data.throttle = dbc_decode(frame, SIG_ENGINE_THROTTLE);
    } else if (frame.id == ECU_ID_BRAKE) {
...
void dashboard_render(void) {
    utils_clear_screen();
    printf("====================================================\n");
    printf("           ADVANCED AUTOMOTIVE DASHBOARD            \n");
    printf("====================================================\n\n");
